...
public class NoteService {
    ...
    public NoteResponse getNote(Long noteId) {
        noteRepository.updateViewCount(noteId); // 조회수 증가
    ...
    public void updateNote(Long noteId, NoteRequest noteRequest) {
        ...
        NoteHistory history = NoteHistory.builder()
                ...
                .content(noteRequest.getContent())
                .build();
        noteHistoryRepository.save(history); // 변경 이력 저장
    ...
    public void deleteNote(Long noteId) {
        ...
        // Note Entity에 @OneToMany(mappedBy = "note", cascade = CascadeType.REMOVE, orphanRemoval = true)
        // 설정으로 인해, Note 삭제 시 관련된 NoteHistory도 함께 삭제되어 아래 코드 불필요
        // noteHistoryRepository.findAllByNoteIdOrderByCreatedDateDesc(noteId)
        //         .ifPresentOrElse(noteHistory -> {
        //             noteHistoryRepository.delete(noteHistory);
        //         }, () -> {
        //             throw new IllegalArgumentException("Note history not found");
        //         });
    }
